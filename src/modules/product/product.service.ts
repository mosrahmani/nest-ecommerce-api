...
export class ProductService {
    ...
    const filtersList = [
      {
        key: 'category_id',
        field: 'category_id',
        where: { equals: +category_id },
      },
      { key: 'status', field: 'status', where: { equals: status } },
      {
        key: ['price_to', 'price_from'],
        field: 'price',
        where: { gte: +price_from || undefined, lte: +price_to || undefined },
      },
    ];
  ...
  // async uploadPicture(id: number, files: Array<Express.Multer.File>): Promise<Product> {
  //   const product = await this.prisma.product.findUnique({ where: { id } })
  //   if (!product) throw new NotFoundException('Product Not Found!');
  //
